...
if __name__ == "__main__":
    ...
    sentences = test_dataset.get_sentences()
    true_tags = test_dataset.get_tags()
    ...
    with torch.no_grad():
        for data in test_dataset:
            ...
            pred_tag.append(
                ...
                )[1: (len(tokenized_sentence[count]) - 1)].tolist() #throw CLS, SEP token at the front and trail
            )
...
inverse_true_tag = [] #inverse lai NER tag in natural language from enc_tag 
for tag in true_tags:
    tag = enc_tag.inverse_transform(np.array(tag))
    inverse_true_tag.append(tag)

print("inverse_tag")

write_to_file(sentences, pred_tag, target_tags = inverse_true_tag)
